...
> **Statut : Phase 0 (DB) ✅ prod · Phase 1 ✅ live · Phase 2 (journal weekly) ✅ LIVE 2026-07-03 — EF v10 déployée, commit 45643a8 poussé, TESTS LIVE 10/10 (spa de test Djerba) : entrée journal chiffré (dual-write `current_value`+blob vérifiés), lien `report_id` stocké et vérifié, SITUATION_REQUIRED / EMPTY_ENTRY / OBJECTIVE_LIMIT_REACHED rejetés, toggle étape → blob current/target recalculé, tag-seul accepté sur type projet, report_id invalide → null (garde-fou). Zéro résidu de test. · Phase 3 ✅ LIVE 2026-07-03 (voir bloc Phase 3 ci-dessous).**
> Phase 2 livrée : EF `add_objective_update` + `toggle_objective_step` (journal transversal aux cycles, PAS de garde is_locked) ; section Objectifs ajoutée au weekly (RapportDetail + AppSidebar) ; SectionObjectifs à deux visages (weekly = point de la semaine : tag 🟢🟠🔴 + valeur + texte + étapes cochables ; monthly = bilan : valeur/statut/commentaire + timeline lecture seule) ; statut de section weekly non bloquant (complete si chaque objectif actif a son entrée liée au rapport, sinon warning) ; dual-write aussi à l'édition manuelle du bilan (`useUpdateObjectiveProgress` patche `current_value`). i18n 1078/1078 ×3.
> Correctifs review Phase 1 appliqués : progression **baseline-relative** (`start` dans le blob + helper `src/lib/objectiveProgress.ts` + 6 lecteurs + 15 tests), types.ts régénéré, action **Clôturer** (page /objectifs), rendu minimal type Projet, titre éditable à la conversion, admin masqué, toast « déjà converti », fix timezone, verrou concurrence trigger + WITH CHECK journal (migration 20260703093000), compensations EF symétriques et loggées, `ids_item_id` renseigné, SectionIdsWeekly (mort) supprimé, copie i18n corrigée.
> **Phase 3 — ✅ LIVE 2026-07-03** (migration garde UPDATE appliquée en prod, 2 EF synthèses déployées, commit 43fb336 poussé sur main). **Probes live 4/4 (Djerba)** : réactivation légitime 2→3 actifs OK ; 4e actif rejeté `OBJECTIVE_LIMIT_REACHED` ; update ordinaire à 3/3 passe (WHEN transition-only vérifié) ; synthèse hebdo régénérée mentionne le journal (entrée 🔴 « fournisseur en rupture » reprise dans l'executive_summary + key_action dédiée). Zéro résidu de test. ⚠️ Découverte : l'URL publiée `spa-flow-pilot.lovable.app` sert un build placeholder d'avril 2026 (aucun code SPA OMS dedans) — l'app réelle vit sur la preview Lovable (authentifiée, sync auto GitHub) ; pour mettre l'URL publique à jour, cliquer **Publish** dans Lovable. Livré : lib partagée `src/lib/objectiveDisplay.ts` (résolution colonnes-first + méta couleurs sémantiques success/warning/destructive + échéance dépassée) + `ObjectiveCardParts.tsx` → les 3 cartes (SectionObjectifs, /objectifs, MeetingView) + PDF weekly + digest Direction (`weeklyException.ts` étendu : colonnes réelles + étapes) rendent désormais les MÊMES chiffres ; onglets Actifs/Historique sur /objectifs (`useClosedObjectives`, cartes clôturées lecture seule) ; badge « échéance dépassée » partout ; densité MeetingView (2 dernières entrées de journal) ; 2 pluriels en dur corrigés (countBadge/itemCount) ; +11 clés i18n (parité 1089×3) ; EF weekly + monthly enrichies (journal + objectifs clôturés de la période dans le prompt) ; migration `20260703160000_objectifs_phase3_update_guard.sql` écrite (trigger UPDATE anti-réactivation au-delà de 3, WHEN sur la transition seulement — un update ordinaire à 3/3 ne déclenche pas).
> ⚠️ **« PDF mensuel » : transféré au chantier refonte Monthly** — il n'existe AUCUN PDF mensuel dans l'app (seul PDF = weekly ; `ReportHeader` ne l'affiche qu'en weekly, DirectionSpaDetail consomme le même). La consultabilité des clôturés est couverte par l'onglet Historique ; la section « objectifs clôturés » naîtra avec le compte-rendu/PDF du chantier Monthly.
> Reste cleanup (non bloquant) : dédup coquille dialogs/mapping, labels FR en dur dans `AppSidebar.weeklyLabelOverrides` (pré-existant), PDF weekly FR-only (dette assumée).
...
> Dernière mise à jour : 2026-07-03 (Phase 3 déployée et vérifiée live — refonte Objectifs terminée).
...
- [x] Les objectifs clôturés restent **consultables** (onglet Historique ✅ ; PDF mensuel inexistant dans l'app → transféré au chantier refonte Monthly).

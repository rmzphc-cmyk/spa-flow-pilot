...
export function SectionCheckin({ reportId, onStatusChange, isLocked = false }: Props) {
  ...
  return (
    <section className="mb-8">
      <h2 className="text-lg font-semibold text-foreground">{t("report.checkin.title")}</h2>
      <p className="text-sm text-muted-foreground mb-4">{t("report.checkin.subtitle")}</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <Field
          label={t("report.checkin.equipe.label")}
          sublabel={t("report.checkin.equipe.sublabel")}
          value={equipeScore}
